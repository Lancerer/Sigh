...
public class CityFragment extends BaseFragment {
    ...
    private List<CityBean.NewslistBean> lists;
    ...
    private void request(final boolean flag) {
        HttpUtil.getRequest("http://api.tianapi.com/world/?key=4e8309c39e4ddab74b0b3ac294030bc3&num=10&rand=1", new Callback() {
            @Override
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                Gson gson = new Gson();
                final CityBean cityBean = gson.fromJson(json, CityBean.class);
                getActivity().runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (!flag) {
                            lists = cityBean.getNewslist();
                            recyCity.setLayoutManager(linearLayoutManager);
                            adapter = new CityAdapter(lists, getContext());
                            ...
                            List<CityBean.NewslistBean> newslist = cityBean.getNewslist();
